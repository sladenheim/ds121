...
(1) addition, and <br>
(2) mulitiplication by a constant.
...
The equation on the right is called a **vector equation**. This single equation that involves column vectors is _**equivalent to**_ a system of linear equations arranged in rows. That is, they have the same solution:
...
::::{style="font-size:.8em"}
If linear systems can be inconsistent, then so can be vector equations.
<br><br>
_**What does that mean for a vector equation to be inconsistent?**_
<br><br>

The answer involves the _**span**_ of a set of vectors.
<br><br>
Let's say we are given vectors $\mathbf{a_1}$, $\mathbf{a_2}$, and $\mathbf{b}$.

Assume that it is possible to express $\mathbf{b}$ as a linear combination of $\mathbf{a}_1$ and $\mathbf{a}_2$. That is, there are some $x_1,x_2$ such that $x_1\mathbf{a_1} + x_2\mathbf{a_2}=\mathbf{b}.$
<br><br>
