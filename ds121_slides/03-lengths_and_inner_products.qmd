...
def generate_html():
    ...
    <span class="label">Properties of inner products</span>
        ...
        <ul>
            ...
            <li><b>\( (\mathbf{u} + \mathbf{v})^{\top} \mathbf{w} = \mathbf{u}^{\top} \mathbf{w} + \mathbf{v}^{\top} \mathbf{w} \)</b> <em>(linearity)</em></li>
            <li><b>\( (c \mathbf{u})^{\top} \mathbf{v} = c (\mathbf{u}^{\top} \mathbf{v}) = \mathbf{u}^{\top} (c \mathbf{v}) \) </b><em>(linearity)</em></li>
            <li><b>\(\mathbf{u}^{\top} \mathbf{u} \geq 0 \)</b><em> (non-negativity)</em></li>
        </ul>
        <p>Note: \( \mathbf{u}^{\top} \mathbf{u} = 0 \) if and only if \( \mathbf{u} = 0 \).</p>
    </div>
    """
...
display(HTML(html_content))
```
<!-- With the inner product at our disposal, let's start talking about vector geometry. -->
...
def generate_html():
    return r"""
    <div class="blue-box">
        <p>Can you explain why the inner product is symmetric?
        </p>
    </div>
    """
...
pi-THUH-gor-ee-uhn THEER-uhm X 2
...
def generate_html():
    ...
    <span class="label">Vector length (norm)</span>
        <p>
        The <b>length</b> (or <b>norm</b>) of a vector \(\mathbf{v} = {\tiny \begin{bmatrix}v_1 \\v_2 \\\vdots \\v_n\end{bmatrix}}\) is the non-negative scalar \(\|\mathbf{v}\|\), defined by: 
...
__Remark__: We also refer to $\|\mathbf{v}\|$ as the magnitude of $\mathbf{v}$. 
...
Remember the non-negativity property: inner product with itself $\geq$ 0!
:::
...
Use the property above with $c=\frac{1}{\|\mathbf{v}\|}$!

Think of 1000 u! Simplify your computations!
...
::::{style="font-size: .8em"}
We say that two vectors $\mathbf{u}$ and $\mathbf{v}$ are perpendicular if and only if they form a right angle at the origin.
...
Draw the line conneting $\mathbf{u}$ and $\mathbf{v}$. Then $\mathbf{u}$ and $\mathbf{v}$ are perpendicular if and only if this forms a right triangle, satisfying the Pythagorean theorem.
::::
## Orthogonality
...
def generate_html():
    return r"""
    <div class="blue-box">
        <p>What is the length of the red side of the triangle?
        </p>
    </div>
    """
...
According to the definitions, we have developed today, it is $\|\mathbf{u} - \mathbf{v}\|$.
...
Hence, the blue and green lines are perpendicular if and only if:
...
Equivalently, 
$$
\cos\theta=\frac{\mathbf{u}^\top}{\|\mathbf{u}\|}\frac{\mathbf{v}}{\|\mathbf{v\|}}.
$$
...
- The length $\mathbf{\|\mathbf{v}\|}$ is the square root of $\mathbf{v}^\top \mathbf{v}$.
- The vector $\frac{\mathbf{v}}{\|\mathbf{v}\|}$ is a unit vector. Its length is 1.
- The inner product $\mathbf{u}^\top \mathbf{v}$ is zero when $\mathbf{u}$ and $\mathbf{v}$ are orthogonal.
- The cosine of $\theta$, the angle between $\mathbf{u}$ and $\mathbf{v}$, is given by $\cos\theta = \mathbf{\frac{\mathbf{u}^\top \mathbf{v}}{\|\mathbf{u}\|\|\mathbf{v}\|}}$.
::: -->