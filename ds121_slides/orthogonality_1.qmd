...
:::{style="font-size: .8em"}
In this lecture, we will deepen our understanding of the geometry of vectors and matrices. 

We will explore the concept of orthogonality between two vectors in greater detail and introduce the notion of projection. Additionally, we will introduce the Gram-Schmidt orthogonalization procedure, which transforms any basis into an orthogonal one. In the future, we will apply the Gram-Schmidt process to perform a new type of matrix factorization known as QR factorization.
:::
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        A set of vectors \(\{\mathbf{u}_1,\dots,\mathbf{u}_p\}\) in \(\mathbb{R}^n\) is said to be an <b>orthogonal set</b> if each pair of distinct vectors from the set is orthogonal, i.e., \(\mathbf{u}_i^\top \mathbf{u}_j = 0\;\;\mbox{if}\;i\neq j.\)
        </p>
    </div>
    """
...
We are going to show that $\{\mathbf{u}_1,\mathbf{u}_2,\mathbf{u}_3\}$ is an orthogonal set, where
:::
:::{style="font-size: .6em"}
...
:::
:::{style="font-size: .8em"}
Hence, $\{\mathbf{u}_1,\mathbf{u}_2, \mathbf{u}_3\}$ is an orthogonal set. 
:::
...
Since, $\mathbf{u}_1^\top \mathbf{u}_2 = 0,$ they form an orthognal basis for subspace $\mbox{Span}\{\mathbf{u}_1,\mathbf{u}_2\}.$
:::
...
- There are $n$ inner products, each requiring $2n$ flops.
...
:::{style="font-size: .8em"}
Let $\mathbf{y} = \begin{bmatrix}7\\6\end{bmatrix}$ and $\mathbf{u} = \begin{bmatrix}4\\2\end{bmatrix}.$  We are going to find the orthogonal projection of $\mathbf{y}$ onto $\mathbf{u}$ and write $\mathbf{y}$ as the sum of a vector in Span$\{\mathbf{u}\}$ and a vector orthogonal to $\mathbf{u}.$
...
:::{style="font-size: .8em"}
The component of $\mathbf{y}$ orthogonal to $\mathbf{u}$ is
...
A decomposition like $\mathbf{y} = c_1\mathbf{u}_1 + \dots + c_p\mathbf{u}_p$ is decomposing $\mathbf{y}$ into _a sum of orthogonal projections onto one-dimensional subspaces._
...
Proof: see the notes for a proof by induction.

## Key Takeaways

:::{style="font-size: .8em"}

- An orthogonal set consists of vectors that are pairwise orthogonal. It is linearly independent.
<!-- 
- An orthogonal basis is a basis that is also an orthogonal set. -->

- The coordinates of $\mathbf{y}$ in the orthogonal basis $\{\mathbf{u}_1,\dots,\mathbf{u}_p\}$ are given by $c_j = \frac{\mathbf{y}^\top \mathbf{u}_j}{\mathbf{u}_j^{\top} \mathbf{u}_j}.$ Finding them is computationally efficient.

- The projection into $\mathbf{u}$ decomposes a vector into the sum of its orthogonal projection into $\mathbf{u}$ and the component orthogonal to $\mathbf{u}$.

- The Gram-Schmidt process is an algorithm that transforms any basis into an orthogonal basis, a basis that is also an orthogonal set.
