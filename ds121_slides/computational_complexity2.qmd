...
:::{style="font-size:.8em"}
- Cost of an algorithm (continued)
    - Big-O notation
    - Parallelization
...
def generate_html():
    ...
    <div class="purple-box">
        ...
        Let \(f\) and \(g\) be real-valued functions. We say that \(f(x)\) is \(O(g(x))\) if there are constants \(c\) and \(k\) so that
        $$|f(x)| \leq cg(x)$$
        for all \(x > k.\)
        </p>
    </div>
...
__Remark__: We read this as "$f$ is big-O of $g$" or "$g$ assymptotically dominates $f$."

__Remark__: The constants $c$ and $k$ are called __witnesses__. Only one pair of witnesses is needed.

__Example__: $f(x) = x^2+3x$ is $O(x^2),$ because for $x>1$, $|x^2+3x| \leq 4 x^2.$
:::
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
        What is the computational cost of matrix multiplication <code>C = A @ B</code>, given two \(n \times n\) matrices?
        </p>
    </div>
    """
html_content = generate_html()
...
:::{style="font-size:.8em"}
_**What is the most efficient way to compute $A^2{\bf x}$?**_
...
:::{style="font-size: 1em"}
 Compare floating point numbers for closeness, not equality
...
_**Two numbers, each with small relative error, can yield a value with large relative error if subtracted.**_ If two numbers have errors in the 9th significant digit, but the first 8 significant digits cancel out, then all of a sudden the relative error is large.
...
Here, the absolute error in the inputs is on the order of $10^{-16}$, but the relative error of the output is on the order of $10^{-9}$ – i.e., a million times larger.
...
Given the $2 \times 2$ matrix $A = \begin{bmatrix} 1 & 2.0000000001 \\ 2 & 4 \end{bmatrix},$ solve:
