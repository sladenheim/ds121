...
Let's denote the matrix of a2D linear transformations as: 
...
def generate_html():
    ...
    <div class="purple-box">
        <p>  
For any linear transformation \(\mathbb{R}^2 \to \mathbb{R}^2\) represented by
...
:::{.fragment}
Recall that when $A$ is the matrix of a projection, the unit square collapses onto the $x$-axis, resulting in a shape with area of zero.
:::
:::
...
:::{style="font-size: .8em;"}
This is confirmed by the determinant:
...
:::{style="font-size: .8em;"}
The determinant can be defined for _any_ square matrix.

Here is a way to calculate the determinant based on the elementary row operations from Gauss-Jordan elimination.
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        Let \(A\) be an \(n\times n\) matrix, and let \(U\) be any echelon form obtained from \(A\) by row replacements and row interchanges -- without row scalings. Let \(r\) be the number of such row interchanges. <br> <br>
...
Hence, $\det (A)$ equals $(-1)^1(1)(-2)(-1) = -2.$  

Any other way of computing the echelon form gives the same determinant.
:::
:::
...
:::{style="font-size: .8em"}
Let $A = \left[\begin{array}{rr}0&-1\\1&0\end{array}\right]$ and $B = \begin{bmatrix} 0 & 1 \\ -1 & 0 \end{bmatrix}$. Show that $B$ is the inverse of $A$.

You can calculate $AB$ and $BA$ algebraically, but it is easier to reason about this problem geometrically. Note that $A$ and $B$ corresponds to counterclockwise rotations by 90 and 270 degrees, respectively. 
...
:::{.column .incremental}
It is _not_ invertible. There are many equivalent ways to reach this conclusion:
