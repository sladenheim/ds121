...
<img src="images/linearity/three-modes.jpeg" width=800 />
...
To solve a linear system, our objective is to transform it into a new system which is equivalent to the old system, meaning it has the same solution set. However, in the new system the solution is explicit.
...
_Fact Number 1_: Given a set of linear equations, we can _change the order_ of the equations without changing anything. For example, the following two linear systems have the same solution set.
...
:::{.fragment}
_Fact Number 2_: We can _multiply any equation by a constant_ without changing its meaning. For example: the following two linear systems have the same solution set.
$$
\begin{array}{rcr}
3x = 2
\end{array}
...
:::{style="font-size:.8em"}
__Fact Number 3__: we can __add one equation to another__ without changing the solution set.

For example, the following two systems of linear equations have the same solution set:
...
:::{.fragment}
The procedure we will use to solve a linear system consists of two steps: __Elimination__ and __Backsubstitution__. Let's recall how it works.
:::
:::
...
:::{style="font-size: .7em"}
The goal of the elimination step is to create an upper triangular matrix (or system) with 1s on the diagonal.

The basic operation we will repeatedly apply is to _add a multiple of one equation (row) to another_. We’ll do this with the equations and the matrix side-by-side.

:::{.fragment}
__Example.__ Here is a linear system written both as a set of equations (on the left) and an augmented matrix (on the right).
...
Our first objective is to change the red $\color{red}{6}$ into a 0. (Some other scalars will change along the way.)
:::

:::{.fragment}
To do so: we use Fact Number 3 that "we can add one equation to another without changing the solution set."
:::
:::
...
We will substitute this result back into row 3 of the linear system. This step can be represented using the notational shorthand
...
:::{style="font-size:.7em"}
Next, we use Fact Number 2 and multiply the second equation by $1/2$ to get its leading coefficient to be 1. This step can be represented with the notational shorthand $\text{row}_2 \gets \frac{1}{2} \cdot \text{row}_2.$
...
:::{.fragment}
Next, we multiply the second equation by $-17$ and add it to the third equation. In other words, we perform the operation $\text{row}_3 \gets \text{row}_3 - 17 \cdot \text{row}_2.$
...
:::{style="font-size: .8em"}
Once we have an upper triangular matrix, the process shifts to __backsubstitution__. We now have the value for one variable, and we will substitute it into other equations to simplify them and get values for the other variables.

Although we think of it as a somewhat different stage, in reality it still comes down to applying the same three rules.
...
:::{style="font-size:.5em"}
Let’s get a sense of this process geometrically. Recall the three starting equations.
$$
\begin{array}{rcr}
    x_1 - 2x_2 +x_3 &=& 5\\
    2x_2 - 8x_3 &=& -4\\
...
:::{style="font-size:.8em"}
Now recall our final set of equations: &emsp;&emsp;&emsp;&emsp;
:::
...
:::{style="font-size: .6em"}
Let us introduce further essential terminology,
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        A <b>pivot position</b> is the location of the first nonzero element in each row. 
        </p>
    </div>
    """
html_content = generate_html()
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        <b>Pivot value</b> is the number in the pivot position.
        </p>
    </div>
    """
html_content = generate_html()
...
def generate_html():
    ...
    <div class="purple-box">
        ...
        <li>Each pivot position of a row is in a column to the right of the pivot position of the row above it.</li>
...
In this diagram, the $\blacksquare$ symbols denote nonzero pivot positions, and the $*$ symbols can be any value.   
:::
:::
...
## Gauss-Jordan Elimination: the Algorithm {.really-really-smaller-title}
...
:::{style="font-size: .6em"}
Let's assess the computational cost required to solve a system of $n$ equations in $n$ unknowns using Gauss-Jordan Elimination. In this case, the augmented matrix $A$ is an $n \times (n+1)$ matrix.  
...
based on the formula about sums of squares i.e., $\left(\lim_{n\rightarrow\infty} \frac{\frac{2}{3} n^3 +n^2 - \frac{5}{3} n}{\frac{2}{3} n^3} = 1.\right)$ 
...
- Gauss-Jordan elimination is a process that solves linear systems using elementary row operations. It has two stages: elimination and backsubstitution.

- Gauss-Jordan elimination can be written as a systematic computer algorithm with flop count of $\frac{2}{3}n^3.$
::: 