...
:::{style="font-size: .8em"}
In the last lecture we discussed bases and dimensions of subaspaces. Today we will procide a geometric interpretation of a basis. In fact, we will see a connection between a basis and a coordiante system of a subspace.
...
:::{style="font-size: .6em"}
Let's look at the geometry of space in a new way, using our new notion of a __basis__.

A geometric coordinate system gives a _unique name_ to each point in the plane (or in any vector space). Traditionally, we label points using the standard basis. 
...
The standard basis spans $\mathbb{R}^n$. But it is far from the only basis that spans $\mathbb{R}^n$; there are plenty of other options, and we can build a geometric system from any of them.
...
__Proof.__ Suppose $\mathcal{B}\ = \{\mathbf{b}_1,\dots,\mathbf{b}_p\}$ is a basis for $H,$ and suppose a vector $\mathbf{x}$ in $H$ can be generated in two ways, say
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
         Suppose the set \(\mathcal{B}\ = \{\mathbf{b}_1,\dots,\mathbf{b}_p\}\) is a basis for the subspace \(H\). 
         </p>
    </div>
    """
...
:::{style="font-size: .5em"}
__Example.__ In $\mathbb{R}^2$, let's look at the point ${\bf x} = \begin{bmatrix}1\\6\end{bmatrix}$ plotted in the standard basis and relative to a new basis:
...
Now using the $\mathcal{B}$-basis, we get _different coordinates_. These are $[\mathbf{x}]_\mathcal{B} = \begin{bmatrix}-2\\3\end{bmatrix}$.
...
Furthermore, it shows that $c_1 = 2$ and $c_2 = 3,$ so
