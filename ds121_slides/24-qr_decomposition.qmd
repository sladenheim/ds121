...
**QR avoids this entirely.** Because $Q$ has orthonormal columns, $Q^\top Q = I$ exactly — no inversion needed. The only system left to solve is $R\hat{\mathbf{x}} = Q^\top \mathbf{b}$, which is already upper triangular.
