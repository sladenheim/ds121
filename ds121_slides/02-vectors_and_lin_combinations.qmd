...
::::{style="font-size: .8em"}
- Purpose: Understand why vectors are important
...
_**Two**_ seperate numbers $v_1$ and $v_2$ form a _**two-dimensional**_ vector:
...
:::{.notes}
Notation: $\vec{v}, \mathbf{v}, \underline{v}, \bar{v},$...

$\vec{0}$ and 0 are, in general, different objects
:::
...
:::{.notes}
Purple box: Definition, theorem, Lemma
...
:::{.notes}
1. Imports the NumPy library, which is commonly used for numerical operations in Python.

2. np. array function  
creates a 2×1 column vector u...

3. array_equal checks whether the arrays u and v are exactly equal in both shape and content.
...
:::{.notes}
Do you know how the head-to-tail method works?

Head: ending point/arrow

Tail: starting point/origin
:::
...
:::{.notes}
light green: q, blue: p, dark green: w, 
\begin{align*}
{\bf w} &= 2{\bf u} + {\bf v}\\
{\bf p} &= -{\bf u} - {\bf v}\\
{\bf q} &= -{\bf u} + 3{\bf v}
...
:::{.notes}
We can construct the full $\mathbb{R}^2$ using u and v.
...
## Linear Combinations
:::{style="font-size: .8em"}
Consider $\bf{u}$ ,$\bf{v}$ and $\bf{w}$ that are vectors in $\mathbb{R}^3$ and none of them is a linear combinaation of the other(s).
