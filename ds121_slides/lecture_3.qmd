...
- the cosine of the _angle_ between two vectors
- vector _orthogonality_
:::
::::
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        Let \(\mathbf{u}, \mathbf{v} \in \mathbb{R}^n\) be \(n\)-dimensional column vectors. 
        The <b>inner product</b> (or <b>dot product</b>) of \(\mathbf{u}\) and \(\mathbf{v}\) 
        is the number \(\mathbf{u}^{\top} \mathbf{v}\) such that:
        </p>
        <p>
        \[
...
The inner product is also denoted by $\mathbf{u} \cdot \mathbf{v}$.
...
$\mathbf{\begin{bmatrix} p_1 \\ p_2 \\ p_3 \end{bmatrix}}$ 
for each unit — this is the price vector $\mathbf{p}$. The quantities we buy or sell are 
$\mathbf{q} = \mathbf{\begin{bmatrix} q_1 \\ q_2 \\ q_3 \end{bmatrix}}$. 
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        The <b>length</b> (or <b>norm</b>) of a vector \(\mathbf{v} = {\tiny \begin{bmatrix}v_1 \\v_2 \\\vdots \\v_n\end{bmatrix}} \in \mathbb{R}^n\) is the non-negative scalar \(\|\mathbf{v}\|\), defined by: 
...
:::{.fragment style="font-size: .8em"}
We also refer to $\|\mathbf{v}\|$ as the magnitude of $\mathbf{v}$. 
:::

:::{.fragment style="font-size: .8em"}
The length of a four-dimensional vector would be $\sqrt{v_1^2 + v_2^2 + v_3^2 + v_4^2}$. That is, $\begin{bmatrix} 1 \\ 1 \\ 1 \\ 1 \end{bmatrix}$ has a length of $\sqrt{1^2 + 1^2 +1^2 +1^2}= 2$.
:::
...
::::{style="font-size: .8em"}
**Example.** Let $\mathbf{v}= {\tiny\begin{bmatrix} 4 \\ -2 \\ 2 \\ 1 \end{bmatrix}}$. Find the unit vector $\mathbf{u} \in \mathbb{R}^3$ in the _same direction_ as $\mathbf{v}$.
...
:::{.fragment}
_Then,_ multiply $\mathbf{v}$ by $1/\|\mathbf{v}\|$ to obtain:
...
Geometrically, the distance between two vectors is the length of their difference.
::::

## Distance

:::{style="font-size: .8em"}
Algebraically, the distance between $\mathbf{u}$ and $\mathbf{v}$ in $\mathbb{R}^n$ follows the formula for Euclidian distance between two points.
$$
\text{dist}(\mathbf{u},\mathbf{v}) = \sqrt{(v_1 - u_1)^2 + (v_2 - u_2)^2 + \dots + (v_n - u_n)^2}.
$$
Which you can see is equal to:

$$
\|\mathbf{u} - \mathbf{v}\| = \sqrt{(\mathbf{u} - \mathbf{v})^\top (\mathbf{u} - \mathbf{v})} = \sqrt{\begin{bmatrix} u_1 - v_1 & \dots & u_n - v_n \end{bmatrix} \begin{bmatrix} u_1 - v_1 \\ \vdots \\ u_n - v_n \end{bmatrix}}.
$$
...
Draw the line conneting $\mathbf{u}$ and $\mathbf{v}$. Then $\mathbf{u}$ and $\mathbf{v}$ are perpendicular if and only if this is a right triangle, meaning that the Pythagorean theorem is satisfied.
::::
## Orthogonality
...
$$
\|\mathbf{u}-\mathbf{v}\|^2 = \|\mathbf{u}\|^2 + \|\mathbf{u}\|^2.
$$
:::
...
$$
\|\mathbf{u} - \mathbf{v} \|^2 = \|\mathbf{u}\|^2 + \|\mathbf{v}\|^2 - \mathbf{2 u^\top v}.
$$
:::
:::{.fragment}
...
:::
:::{.fragment}
Thus, if $\mathbf{u}$ is orthogonal to $\mathbf{v}$, then $\mathbf{u}^\top \mathbf{v}$.
:::
:::
...
:::{.fragment}
The zero vector $\mathbf{0}$ is perpendicular to every vector $\mathbf{v}$ because $\mathbf{0}^\top\mathbf{v}$ is always zero.
:::

:::{.fragment}
Now suppose $\mathbf{u}^\top\mathbf{v}$ is not zero. If $\mathbf{u}^\top\mathbf{v}$ is _positive_, the angle is less than $90^\circ$. If $\mathbf{u}^\top\mathbf{v}$ is _negative_, the angle is above $90^\circ$.
:::
::::
