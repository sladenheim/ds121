...
:::{style="font-size: 1.2em" .center-y}
...
:::{.fragment}
In this class, we think of the data $\mathbf{x}$ as a vector. We refer to the components of $\mathbf{x}$ as **features**.
:::
:::


## Supervised Learning
:::{style="font-size: 0.8em" .center-y}
...
::::{style="font-size: .8em" .center-y}
$k$NN is an example of a supervised, non-parametric classifier.

:::{.fragment}
Like any classifier, $k$NN is trained by providing it a set of labeled data, and the goal is to classify future data points that aren't already labeled.
...
::::{style="font-size: .8em" .center-y}
The _curse of dimensionality_ refers to serious issues that occur when using geometric algorithms in high dimensions.
...
$k$-NN relies on having "close" points to the test point $x$, requiring _dense training data_. 
...
- The main difference between parametric and non-parametric models is that parametric models have a fixed number of parameters no matter the size of the training set.

- Classification algorithms work with data that has discrete labels.

- $k$-NN is an example of a supervised, non-parametric classifier. It classifies new unlabeled points by looking at its $k$ closest neighbors.
