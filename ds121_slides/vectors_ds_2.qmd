...
:::{.fragment}
In this lecture we will also touch upon the concepts of similarity and dissimiliarity mesuares, synthetic data, and multidimensional scaling.
:::
::::
...
:::{.fragment}
We are naturally interested in how _similar_ or _dissimilar_ two objects are.
:::
::::
...
One of the most common is that it be a _metric_.

A metric is commonly called a _distance_. We will mainly work with Euclidian distance.
::::
...
This value is _large_ when $\mathbf{x} \approx \mathbf{y}.$  So it is a _similarity_ function.
:::
<br>
...
A clustering algorithm _minimizes_ intra-cluster distances and _maximizes_ inter-cluster distances.
...
::::{style="font-size: .8em"}
In this course, we will focus on _partitional clustering_.

:::{.fragment}
In a **partitional clustering**, the points are divided into a set of _non-overlapping_ groups. That is, each point belongs only to one cluster, and the set of clusters covers all points.
...
Now _cluster_ the pixels by their color value, and replace each pixel by its _cluster center_. Because there are a smaller number of colors used, we can use fewer bits for each pixel.
...
:::{.fragment}
_Nonetheless,_ there is a simple algorithm that works quite well in practice!


This algorithm was voted among the _top-10 algorithms_ in data mining!
...
2. For each $j$ between 1 and $k$, define the cluster $X_j$ as the set of points in $X$ that are _closest to center_ ${\bf c_j}$.
3. For each $j$ between 1 and $k$, redefine ${\bf c_j}$ to be the _center of mass_ of cluster $X_j$.
...
::::{style="font-size: .8em"}
Geometrically, the dataset lives in a _30 dimensional_ space, so we cannot directly visualize it. But we can approximate it!

__Multidimensional Scaling__ (MDS) is a way to visualize the relative similarity or dissimilarity between pairs of points in a dataset.

:::{.fragment}
The idea behind MDS is:

given a distance (or dissimilarity) matrix, find a set of coordinates for the points that approximates those distances as well as possible.
:::
