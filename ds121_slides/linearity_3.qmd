...
However, we can think of the matrix equation $A({\bf x}) = {\bf b}$ in a new way:

:::{.fragment} 
We will think of $A$ as a function that is "acting on" the vector $\bf x$ to create a new vector $\bf b$.
...
:::{style="font-size: .7em"}
For example, consider $A = \left[\begin{array}{rrr}2&1&1\\3&1&-1\end{array}\right].$ We will then find: $A \left[\begin{array}{r}1\\-4\\-3\end{array}\right] = \left[\begin{array}{r}-5\\2\end{array}\right]$
...
:::{.center-text}
<img src="images/linearity/1d-3d-transform.jpeg" width = 800/>
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        A <btransformation</b> (or <b>function</b> or <b>mapping</b>) \(T\) from \(\mathbb{R}^n\) to \(\mathbb{R}^m\) is a rule that assigns to each vector \({\bf x}\) in \(\mathbb{R}^n\) a vector \(T({\bf x})\) in \(\mathbb{R}^m\).<br>
...
We sometimes write that $T$ maps ${\bf x} \mapsto T({\bf x})$.
:::
...
$T$'s _domain_ is $\mathbb{R}^2$, codomain is $\mathbb{R}^3$, and _range_ is the green plane.
...
:::{style="font-size: .8em"}
I have these points in $\mathbb{R^2}$:
...
This definition captures a _lot_ of transformations that are not matrix-vector multiplication. For example:
...
:::{style="font-size: .7em"}
__Example.__ Given a scalar $r$, define $T: \mathbb{R}^2 \rightarrow \mathbb{R}^2$ by $T(\mathbf{x}) = r\mathbf{x}$.  
...
:::{.fragment}
__Solution.__ Let $\mathbf{u}, \mathbf{v}$ be in $\mathbb{R}^2$ and let $c, d$ be scalars.   Then
...
:::{style="font-size: .8em"}
__Example.__ Let $T(\mathbf{x}) = \mathbf{x} + \mathbf{b}$ for some $\mathbf{b} \neq 0$. To give a concrete example, here is one such transformation in two-dimensional space:  $T(\mathbf{x}) = \mathbf{x} + \begin{bmatrix}1 \\ 2\end{bmatrix},$
...
:::{.fragment}
If $\mathbf{b} \neq 0$, those two expressions are not equal.

So $T$ is _not_ a linear transformation.
:::
:::
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
        Let \(T: \mathbb{R}^n \rightarrow \mathbb{R}^m\) be a linear transformation.   Then there is (always) a unique matrix \(A\) such that:<br>
...
:::{.fragment}
Let ${\bf e_j}$ denote the $j$th column of $I$ in $\mathbb{R}^n$. Recall from DS-120 that these vectors are the __standard basis vectors__.
...
:::{.fragment}
$$
\textcolor{white}{T({\bf x}) ===} = \left[T({\bf e_1}) \dots T({\bf e_n})\right] \, \left[\begin{array}{r}x_1\\\vdots\\x_n\end{array}\right] = A{\bf x}.
$$
:::
:::
...
The above theorum tells us that to find the standard matrix of a linear transformation, ask what the transformation does to the columns of $I.$

:::{.fragment}
Let us apply this to rotations about the origin.
...
:::{style="font-size: .5em"}
**Example.** Let $T: \mathbb{R}^2 \rightarrow \mathbb{R}^2$ be the transformation that rotates each point in $\mathbb{R}^2$ about the origin through an angle $\theta$, with counterclockwise rotation for a positive angle.  
...
:::{.fragment}
To demonstrate computationally the use of a rotation matrix, let's rotate the following shape `note`. It is constructed using an array of 26 vectors in $\mathbb{R}^2$. In other words, it is a 2 $\times$ 26 matrix.
...
In two dimensions, the standard basis $V$ is given by ${\bf e_1} = \left[\begin{array}{r}1\\0\end{array}\right]$ and ${\bf e_2} = \left[\begin{array}{r}0\\1\end{array}\right].$
...
def generate_html():
    ...
    <div class="purple-box">
        <p>
        Where does \(\) with \(r=2.5\) map the standard basis vectors?
        </p>
    </div>
    """
html_content = generate_html()
...
Next, let's reflect through the x_1 axis. To find the standard matrix $A,$ we ask where $\bf e_1$ and $\bf e_2$ go?
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
        What about 180-degree rotation through the origin? Why is this different than the reflection about the line \(x_1 = -x_2\)?
...
:::{style="font-size: .6em"}
We know matrix-vector multiplication should be interpreted as a linear transformation over $\mathbb{R^n}.$
...
:::{style="font-size: .6em" .incremental}
However, we could have done this differently:

1. Reflect through the $x_1$ axis, via multiplication by $B = \left[\begin{array}{rr}1&0\\0&-1\end{array}\right].$

2. Reflect throuh the $x_2$ axis, via multiplication by $A = \left[\begin{array}{rr}-1&0\\0&1\end{array}\right].$
...
:::{style="font-size: .6em"}
So, another way to reflect point ${\bf u}$ through the origin is:

* ${\bf v} = B{\bf u}$
* Followed by ${\bf w} = A{\bf v}.$

:::{.fragment}
This is, ${\bf w} = A(B{\bf u}).$ We also know ${\bf w} = C {\bf u}$, which is true for all vectors ${\bf u}$.
:::
...
:::{.fragment}
Just like functions: the order matters, and we read them __from right to left__!
:::
:::
