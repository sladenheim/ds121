...
Today, we will introduce Markov chains, special mathematical systems where your current state depends solely on your previous state. We will define them from a linear algebra perspective, connecting these systems to vectors and matrices.

In addition, we will explore special properties of Markov chains called steady states. Steady states are unique conditions where the probabilities of being in each state remain constant over time, regardless of the initial state.  We will describe how to find these steady states, providing insights into how Markov chains behave in the long run.
...
In the early 1900s, Andrei Markov introduced the study of _stochastic systems in which one transitions between states, and the future state depends only on the current state_. 
...
Markov chains have proven to be incredibly useful. For instance, they are used for analyzing stock price movements, the dynamics of animal populations, and the PageRank algorithm.
:::
...
:::{style="font-size: .8em"}
Mathematically, we define the state of the system at time $k$ is a vector ${\bf x_k} \in \mathbb{R}^n,$ and
...
The stochastic matrix $P$ describes how things "redistribute" themselves at each time step.
:::
...
It can be shown that _every stochastic matrix has at least one steady-state vector._ 
